 ...
 * skipped unless the runner argument `tinytuya.lightCapabilityProbe=true` is present. Never run
 * it through `connectedDebugAndroidTest`: that Gradle deployment may remove the target debug app
 * and its private data at teardown.
 *
 ...
 * 3. `adb shell am instrument -w -e class com.prfd.tinytuya.LightCapabilityProbeInstrumentedTest
 *    -e tinytuya.lightCapabilityProbe true
...
class LightCapabilityProbeInstrumentedTest {
    ...
    fun writeSanitizedLightCapabilityReportToTargetCache() = runBlocking {
        ...
        val catalog = EncryptedDeviceCatalogStore(context).load()
            ?: error("The encrypted device catalog is unavailable.")
        val lights = catalog.devices.filter { device ->
            device.category.trim().lowercase() == "dj"
        }
        assertTrue("Expected one category-dj light.", lights.size == 1)
        val light = lights.single()
        val status = catalog.localStatus.firstOrNull { record -> record.id == light.id }
    ...
    private fun sanitizedObservedText(
        ...
        val shape = JSONObject()
            ...
            .put("hex", value.matches(HEX_VALUE))
        if (code == "work_mode") {
            val range = parsedValues(definition.opt("values"))?.optJSONArray("range")
            val declared = range != null && (0 until range.length()).any { index ->
                range.optString(index) == value
            }
    ...
    private fun sanitizedValues(rawValues: Any?): JSONObject {
        ...
        safeShortText(values.optString("unit"))
            .takeIf(String::isNotEmpty)
            ?.let { unit -> sanitized.put("unit", unit) }
        values.optJSONArray("range")?.let { range ->
            val sanitizedRange = JSONArray()
            for (index in 0 until minOf(range.length(), MAX_ENUM_VALUES)) {
                safeShortText(range.optString(index))
                    .takeIf(String::isNotEmpty)
                    ?.let(sanitizedRange::put)
            }
    ...
    private fun parsedValues(rawValues: Any?): JSONObject? = when (rawValues) {
    ...
    private fun safeCode(value: String): String? = value
        .trim()
        .lowercase()
        .takeIf { code -> code.length in 1..MAX_CODE_LENGTH && code.matches(CODE_VALUE) }

    private fun safeShortText(value: String): String = value
        .take(MAX_SHORT_TEXT_LENGTH)
    ...
    private companion object {
        ...
        val LIGHT_CAPABILITY_CODES = setOf(
            "switch_led",
            "work_mode",
            "bright_value",
            "bright_value_v2",
