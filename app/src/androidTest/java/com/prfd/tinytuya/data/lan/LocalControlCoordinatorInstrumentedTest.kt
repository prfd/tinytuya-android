...
class LocalControlCoordinatorInstrumentedTest {
          ...
          localStatus =
            listOf(
              coverCatalog()
                .localStatus
                .single()
                .copy(dataPoints = listOf(LocalDataPoint("7", LocalDataPointKind.STRING, "pause")))
            ),
        )
    val stale =
      coverCatalog()
  ...
  fun staleLightStatusCannotAuthorizeAWrite() = runBlocking {
    val catalog =
      lightCatalog()
        .copy(
          localStatus = listOf(lightCatalog().localStatus.single().copy(polledAtEpochMillis = 9L))
        )
    val gateway = FakeGateway { request -> confirmedResult(request.changes.single()) }
    val coordinator =
      DefaultLocalControlCoordinator(
  ...
  fun changedWifiIsRejectedBeforeThePythonBridge() = runBlocking {
    ...
    val coordinator =
      DefaultLocalControlCoordinator(
        gateway = gateway,
        catalogStore = FakeStore(catalog),
        networkResolver = FakeNetworkResolver(NETWORK.copy(localIpv4 = "192.168.10.6")),
      )
  ...
  fun differentAndroidNetworkHandleIsRejectedEvenWhenSubnetIsIdentical() = runBlocking {
    ...
    val coordinator =
      DefaultLocalControlCoordinator(
        ...
        catalogStore = FakeStore(catalog),
        networkResolver =
          FakeNetworkResolver(NETWORK.copy(networkHandle = NETWORK.networkHandle + 1)),
      )
  ...
  fun changedDiscoveryGenerationRejectsAQueuedIntent() = runBlocking {
    ...
    val rediscovered =
      original.copy(
        ...
        lanDevices = listOf(original.lanDevices.single().copy(lastSeenAtEpochMillis = 11L)),
        localStatus = listOf(original.localStatus.single().copy(polledAtEpochMillis = 11L)),
      )
    val gateway = FakeGateway { confirmedResult(false) }
    val coordinator =
      DefaultLocalControlCoordinator(
  ...
  fun unverifiedBooleanMappingCannotAuthorizeAWrite() = runBlocking {
    val catalog =
      sampleCatalog(mappingJson = "{\"1\":{\"code\":\"countdown_1\",\"type\":\"Boolean\"}}")
    val gateway = FakeGateway { confirmedResult(false) }
    val coordinator =
      DefaultLocalControlCoordinator(
        gateway = gateway,
          ...
          devices =
            listOf(
              sampleCatalog()
                .devices
                .single()
                .copy(mappingJson = "{\"9\":{\"code\":\"switch_1\",\"type\":\"Boolean\"}}")
            ),
          localStatus =
            listOf(
              sampleCatalog()
                .localStatus
                .single()
                .copy(dataPoints = listOf(LocalDataPoint("9", LocalDataPointKind.BOOLEAN, "true")))
            ),
        )
    val gateway = FakeGateway { request -> confirmedResult(request.changes.single()) }
    val coordinator =
  ...
  private class FakeStore(private var catalog: DeviceCatalog) : DeviceCatalogStore {
    var mergeCount = 0
  ...
  private companion object {
    ...
    fun sampleCatalog(
      mappingJson: String = "{\"1\":{\"code\":\"switch_1\",\"type\":\"Boolean\"}}",
      category: String = "kg",
    ) =
      DeviceCatalog(
        schemaVersion = 3,
    ...
    fun confirmedResult(value: Boolean) =
      LocalControlResult(
        contractVersion = 1,
        id = DEVICE_ID,
        ...
        errorCode = "",
        durationMillis = 35L,
        dataPoints = listOf(LocalDataPoint("1", LocalDataPointKind.BOOLEAN, value.toString())),
      )

    fun confirmedResult(change: LocalControlChange) =
      LocalControlResult(
        contractVersion = 1,
        id = DEVICE_ID,
        state = LocalControlState.CONFIRMED,
        errorCode = "",
        durationMillis = 35L,
        dataPoints = listOf(LocalDataPoint(change.id, change.kind, change.value)),
      )
