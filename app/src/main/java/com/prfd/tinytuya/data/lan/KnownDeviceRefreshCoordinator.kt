...
/**
 * Reads status from addresses verified by the most recent discovery, without opening UDP
 * discovery ports. The Android network identity must still match exactly before any saved address
 * is reused.
 */
class DefaultKnownDeviceRefreshCoordinator(
    ...
    private fun discoveryRequired() = LocalStatusException(
...
private fun DeviceCatalog.statusEligibleDevices() = devices
    .asSequence()
