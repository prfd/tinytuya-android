...
internal fun inspectLocalDataPoints(
  ...
  val inspected =
    dataPoints
      .sortedWith(
        compareBy<LocalDataPoint> { it.id.toIntOrNull() ?: Int.MAX_VALUE }.thenBy { it.id }
      )
      .map { dataPoint ->
...
private fun inspectionValue(
  ...
  when (dataPoint.kind) {
    LocalDataPointKind.BOOLEAN ->
      dataPoint.value.takeIf { value -> value == "true" || value == "false" }
        ?: "Invalid Boolean hidden"
    LocalDataPointKind.INTEGER,
    LocalDataPointKind.DECIMAL ->
      dataPoint.value.takeIf { value ->
...
private val SENSITIVE_CODE_SEGMENTS =
  setOf(
    "auth",
    "credential",
    "key",
