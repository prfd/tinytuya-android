...
internal object LocalRefreshDiagnostics {
    ...
    fun refreshStarted(trigger: LocalRefreshTrigger, mode: LocalRefreshMode): RefreshTrace {
        Log.i(
            TAG,
    ...
    private fun safeCode(code: String): String = when {
        code.isEmpty() -> "none"
        stableCodePattern.matches(code) -> code
        else -> "REDACTED_INVALID_CODE"
    }

    private fun safeReason(reason: String): String = when (reason) {
        "busy",
        "cooldown",
        "disabled",
        "invalid_destination",
