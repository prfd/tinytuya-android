 ...
 * [Context.getNoBackupFilesDir]. Its non-exportable Android Keystore key has a dedicated alias,
 * allowing credentials to be forgotten without deleting the locally usable device catalog.
 */
class EncryptedCloudCredentialStore
internal constructor(
  ...
  override suspend fun load(): StoredCloudCredentials? =
    withContext(Dispatchers.IO) { mutex.withLock { loadLocked() } }

  override suspend fun loadSummary(): CloudCredentialSummary? =
    withContext(Dispatchers.IO) { mutex.withLock { loadLocked()?.toSummary() } }
  ...
  private fun encode(credentials: StoredCloudCredentials): String =
    JSONObject()
      .apply {
  ...
  private fun normalizeAndValidate(credentials: StoredCloudCredentials): StoredCloudCredentials {
  ...
  private fun StoredCloudCredentials.toSummary() =
    CloudCredentialSummary(
      region = region,
  ...
  private companion object {
    ...
    val ASSOCIATED_DATA = "tinytuya-cloud-credential-vault-v1".toByteArray(StandardCharsets.UTF_8)
  }
}

class InMemoryCloudCredentialStore(initial: StoredCloudCredentials? = null) : CloudCredentialStore {
