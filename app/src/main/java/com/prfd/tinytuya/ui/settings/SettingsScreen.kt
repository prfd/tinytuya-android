...
fun SettingsScreen(
  ...
  Surface(
    ...
    Box(
      ...
      LazyColumn(
        modifier = Modifier.fillMaxSize().widthIn(max = 680.dp).testTag("settings_list"),
...
private fun CloudAccountCard(
  ...
  OutlinedCard(
    ...
    modifier = Modifier.fillMaxWidth().testTag("cloud_account_card"),
  ) {
      ...
      when (state) {
        ...
        CloudAccountUiState.Missing -> {
          ...
          Button(
            onClick = onUpdate,
            modifier =
              Modifier.fillMaxWidth().padding(top = 14.dp).testTag("save_cloud_credentials"),
          ) {
        ...
        is CloudAccountUiState.Saved -> {
          ...
          Text(
            text =
              "The secret stays encrypted and is never shown here. Cloud access happens only when you explicitly sync.",
            style = MaterialTheme.typography.bodyMedium,
            color = MaterialTheme.colorScheme.onSurfaceVariant,
          ...
          Button(
            onClick = onSync,
            modifier = Modifier.fillMaxWidth().padding(top = 14.dp).testTag("sync_from_tuya"),
          ) {
            Text("Sync from Tuya")
          }
          Row(
            ...
            OutlinedButton(
              onClick = onUpdate,
              modifier = Modifier.weight(1f).testTag("update_cloud_credentials"),
            ) {
        ...
        is CloudAccountUiState.Recovery -> {
          ...
          OutlinedButton(
            onClick = onForget,
            modifier =
              Modifier.fillMaxWidth().padding(top = 14.dp).testTag("forget_broken_credentials"),
          ) {
...
private fun TinyTuyaHealthCard(state: TinyTuyaHealthUiState) {
  ...
  OutlinedCard(
    colors =
      CardDefaults.outlinedCardColors(containerColor = MaterialTheme.colorScheme.surfaceVariant),
    border = BorderStroke(1.dp, MaterialTheme.colorScheme.outlineVariant),
    modifier = Modifier.fillMaxWidth().testTag("tinytuya_health_card"),
  ) {
    Column(Modifier.padding(18.dp)) {
      Row(
        modifier = Modifier.fillMaxWidth(),
...
private fun SettingsErrorCard(
  ...
  OutlinedCard(
    ...
    Column(
      ...
      TextButton(onClick = onDismiss, modifier = Modifier.padding(top = 4.dp)) { Text("Dismiss") }
    }
  }
}
