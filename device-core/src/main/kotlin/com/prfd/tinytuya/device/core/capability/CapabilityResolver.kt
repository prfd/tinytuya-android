...
object CapabilityResolver {
  ...
  private fun resolveRange(
    ...
    val displayValue =
      when (spec.display) {
        ...
        MeasurementDisplay.PERCENTAGE ->
          PercentageCapabilityCodec.normalize(
  ...
  private fun resolveChoice(
    ...
    if (
      definition.declaredType != DpDeclaredType.ENUM || point.kind != ObservedDataPointKind.STRING
    )
      return null
  ...
  private fun resolveMeasurement(
    ...
    if (definition.declaredType !in NUMERIC_DECLARED_TYPES || point.kind !in NUMERIC_OBSERVED_TYPES)
      return null
