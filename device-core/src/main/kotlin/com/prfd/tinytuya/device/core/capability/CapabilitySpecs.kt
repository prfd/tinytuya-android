...
data class ChoiceCapabilitySpec(
  ...
  init {
    ...
    require(
      choices.isNotEmpty() &&
        choices.size <= MAX_OPTIONS &&
        choices.distinctBy { it.wireValue }.size == choices.size
    ) {
...
data class ActionGroupCapabilitySpec(
  ...
  init {
    ...
    require(
      actions.isNotEmpty() &&
        actions.size <= MAX_OPTIONS &&
        actions.distinctBy { it.wireValue }.size == actions.size
    ) {
...
data class BinaryStateCapabilitySpec(
  ...
  init {
    ...
    require(
      states.isNotEmpty() &&
        states.size <= MAX_OPTIONS &&
        states.distinctBy { it.wireValue }.size == states.size
    ) {
...
private fun validateSpec(label: String, codes: List<String>) {
  ...
  require(
    codes.distinct().size == codes.size &&
      codes.all { code -> code.length in 1..MAX_CODE_LENGTH && CODE.matches(code) }
  ) {
