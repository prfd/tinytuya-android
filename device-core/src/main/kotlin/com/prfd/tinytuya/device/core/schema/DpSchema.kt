...
data class DpDefinition internal constructor(
    val id: String,
    val code: String?,
...
enum class DpDeclaredType {
    ...
    companion object {
        internal fun normalize(value: String?): DpDeclaredType = when (value?.trim()?.lowercase()) {
            "boolean" -> BOOLEAN
            "integer" -> INTEGER
            "value" -> VALUE
            "float" -> FLOAT
...
data class DpConstraints internal constructor(
 ...
 */
class DpSchema private constructor(
    ...
    companion object {
        ...
        fun normalize(inputs: Iterable<DpDefinitionInput>): DpSchema {
            ...
            boundedInputs.forEach { input ->
                ...
                definitionsById[id] = DpDefinition(
                    id = id,
            ...
            return DpSchema(
                definitions = definitionsById.values.sortedWith(
                    compareBy<DpDefinition> { definition ->
                        definition.id.toLongOrNull() ?: Long.MAX_VALUE
        ...
        private fun normalizeCode(value: String?): String? = value
            ?.trim()
            ?.lowercase()
            ?.takeIf { code -> code.length in 1..MAX_CODE_LENGTH && CODE.matches(code) }
