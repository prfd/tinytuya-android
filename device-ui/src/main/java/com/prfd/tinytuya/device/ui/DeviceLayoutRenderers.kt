...
class DeviceLayoutRendererRegistry(renderers: List<DeviceLayoutRenderer>) {
    ...
    init {
        require(byLayoutId.size == this.renderers.size) {
            "Device layout renderer IDs must be unique."
        }
    }
    ...
    internal fun prepare(device: DeviceUiModel): PreparedDeviceLayout? {
        ...
        val consumedIds = runCatching { renderer.prepare(device)?.toSet() }
            .getOrNull()
            ?.takeIf { ids -> ids.isNotEmpty() && ids.all(availableIds::contains) }
            ?: return null
...
fun DeviceLayoutHost(
    ...
    Column(modifier = modifier) {
        ...
        val remaining = device.capabilities.filter { capability ->
            capability.id !in consumedCapabilityIds
        }
