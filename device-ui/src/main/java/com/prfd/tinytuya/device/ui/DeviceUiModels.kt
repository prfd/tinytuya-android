...
internal constructor(
  ...
  init {
    ...
    require(
      this.capabilities.map(CapabilityUiModel::id).distinct().size == this.capabilities.size
    ) {
...
object DeviceUiMapper {
  fun map(device: ResolvedDevice): DeviceUiModel =
    DeviceUiModel(
      deviceId = device.deviceId,
      layoutId = device.layoutId,
      capabilities =
        device.capabilities.capabilities.mapNotNull { capability ->
          when (capability) {
            is ResolvedToggle ->
              ToggleUiModel(
                capability.id,
            ...
            is ResolvedChoice ->
              ChoiceUiModel(
                ...
                capability.choices.map { choice -> ChoiceUiOption(choice.wireValue, choice.label) },
              )
            is ResolvedActionGroup ->
              ActionGroupUiModel(
                capability.id,
                capability.label,
                capability.writable,
                capability.currentWireValue,
                capability.actions.map { action -> ChoiceUiOption(action.wireValue, action.label) },
              )
            is ResolvedColor ->
              ColorUiModel(
                capability.id,
            ...
            is ResolvedMeasurement ->
              MeasurementUiModel(
                capability.id,
                capability.label,
                capability.displayValue,
              )
            is ResolvedBinaryState ->
              BinaryStateUiModel(
                capability.id,
                capability.label,
                capability.value,
